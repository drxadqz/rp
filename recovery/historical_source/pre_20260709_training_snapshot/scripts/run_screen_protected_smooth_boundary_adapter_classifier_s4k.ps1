...
$OutDir = Join-Path $Root "screen_protected_smooth_boundary_adapter_classifier_s4k_e1_20260703"

& $Python scripts\run_rscd_surface_classification.py `
    --output-dir $OutDir `
    --backbone convnext_tiny_protected_smooth_boundary_stem `
    --embedding-dim 768 `
    --image-size 192 `
    ...
    --train-only-module-prefix backbone.protected_smooth_boundary_adapters `
    --train-only-module-prefix classifier `
    --hflip-consistency-weight 0.001 `
    --relation-conditional-weight 0.004 `
