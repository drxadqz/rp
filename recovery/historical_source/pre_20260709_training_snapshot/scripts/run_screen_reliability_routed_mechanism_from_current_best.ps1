...
$OutDir = Join-Path $Root "screen_reliability_routed_mechanism_stem_s4k_cap120_from_current_best_20260703"

& $Python scripts\run_rscd_surface_classification.py `
    --output-dir $OutDir `
    --backbone convnext_tiny_reliability_routed_mechanism_stem `
    --embedding-dim 768 `
    --image-size 192 `
    ...
    --train-only-module-prefix backbone.reliability_routed_mechanism_adapters `
