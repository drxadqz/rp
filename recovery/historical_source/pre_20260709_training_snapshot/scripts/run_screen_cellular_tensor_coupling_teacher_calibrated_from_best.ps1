...
$OutDir = Join-Path $Root "screen_cellular_tensor_coupling_teacher_calibrated_stem0_s8k_e2_from_best_20260702"

& $Python scripts\run_rscd_surface_classification.py `
    --output-dir $OutDir `
    --backbone convnext_tiny_cellular_tensor_coupling_stem `
    --embedding-dim 768 `
    --image-size 192 `
    ...
    --train-only-module-prefix backbone.cellular_tensor_coupling_banks `
    --online-teacher-checkpoint $ResumeFrom `
    --online-teacher-weight 0.03 `
    --online-teacher-temperature 2.0 `
    --online-teacher-beta 4.0 `
    --online-teacher-min-confidence 0.45 `
    --teacher-error-replay-weight 0.10 `
    --teacher-error-replay-focus concrete_wetwater_rough_v1 `
    --teacher-error-replay-beta 1.5 `
    --teacher-error-replay-min-confidence 0.45 `
