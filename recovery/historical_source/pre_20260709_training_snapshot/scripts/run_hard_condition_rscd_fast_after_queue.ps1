...
$Log = Join-Path $LogDir "fast_hard_condition_after_queue.log"
...
Write-QueueLog "started hard-condition RSCD fast queue"
Wait-ProcessIds -Ids $FormalWaitPids -Name "formal RSCD jobs"
Wait-ProcessIds -Ids $WatcherWaitPids -Name "existing RSCD watchers"
Wait-ClassificationJobsGone

Run-FastCandidate `
    -Name "fast_physics_texture_hard_condition_boost035" `
    -ExtraArgs @("--hard-condition-boost", "0.35")

Run-FastCandidate `
    -Name "fast_physics_texture_hard_condition_hier_boost035" `
    -ExtraArgs @("--hard-condition-boost", "0.35", "--hierarchical-smoothing", "0.08")
...
Write-QueueLog "hard-condition candidates complete"
