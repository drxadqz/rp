...
$Log = Join-Path $LogDir "fast_wavelet_texture_after_queue.log"
...
Write-QueueLog "started wavelet-texture RSCD fast queue"
Wait-ProcessIds -Ids $WaitPids -Name "existing RSCD/direct/residual/film queue"
...
Run-FastCandidate `
    ...
    -ExtraArgs @("--use-directional-texture-branch", "--directional-texture-dim", "48", "--use-texture-gate", "--hierarchical-smoothing", "0.08")
...
Write-QueueLog "wavelet-texture candidates complete"
