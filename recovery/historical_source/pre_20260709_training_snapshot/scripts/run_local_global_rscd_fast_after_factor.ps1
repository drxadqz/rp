...
$Log = Join-Path $LogDir "fast_local_global_after_factor.log"
...
function Invoke-Candidate([string]$Name, [string[]]$ExtraArgs) {
...
Write-QueueLog "started local-global RSCD fast watcher"

Wait-File -Path (Join-Path (Join-Path $Root "fast_convnext_tiny_224_factor_aux") "evaluate_test.json") -Name "224px factor-aux ConvNeXt fast result"
Wait-File -Path (Join-Path (Join-Path $Root "fast_physics_texture_quality_224_patch_factor_aux") "evaluate_test.json") -Name "224px factor-aux PhysicsTexture fast result"

Invoke-Candidate -Name "fast_convnext_tiny_lg_224" -ExtraArgs @()

Invoke-Candidate -Name "fast_physics_texture_lg_224_patch_factor_aux" -ExtraArgs @(
    "--use-physics-branch",
    "--physics-quality-cues",
    "--no-physics-quality-region-cues",
    "--physics-dim", "96",
    "--factor-aux-weight", "0.15"
)
...
Write-QueueLog "local-global RSCD fast candidates complete"
