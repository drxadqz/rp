...
$OutDir = Join-Path $Root "screen_bccn_wetconcrete_unfreeze_stemblock_s4k_cap120_from_current_best_20260703"

& $Python scripts\run_rscd_surface_classification.py `
    --output-dir $OutDir `
    --backbone convnext_tiny_bccn_wetconcrete_stem `
    --embedding-dim 768 `
    --image-size 192 `
    ...
    --train-only-module-prefix backbone.boundary_charted_coupling_norms `
    --train-only-module-prefix backbone.features.0 `
    --train-only-module-prefix backbone.features.1.0.layer_scale `
    --train-only-module-prefix backbone.features.1.0.block.0 `
    --train-only-module-prefix backbone.features.1.0.block.2 `
