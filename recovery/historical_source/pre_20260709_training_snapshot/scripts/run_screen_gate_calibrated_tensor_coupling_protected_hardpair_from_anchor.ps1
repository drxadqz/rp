...
$OutDir = Join-Path $Root "screen_gate_calibrated_tensor_coupling_protected_hardpair_s8k_e2_from_anchor_20260702"

& $Python scripts\run_rscd_surface_classification.py `
    --output-dir $OutDir `
    --backbone convnext_tiny_gate_calibrated_tensor_coupling_protected_stem `
    --embedding-dim 768 `
    --image-size 192 `
    ...
    --train-only-module-prefix backbone.gate_calibrated_tensor_coupling_banks `
    --hard-pair-sampling `
    --hard-pair-sampling-fraction 0.85 `
    --online-teacher-checkpoint $Anchor `
    --online-teacher-weight 0.7 `
    --online-teacher-temperature 2.0 `
    --online-teacher-beta 4.0 `
    --online-teacher-min-confidence 0.45 `
    --teacher-error-replay-weight 0.25 `
    ...
    --relation-conditional-weight 0.006 `
    --relation-conditional-focus hard `
