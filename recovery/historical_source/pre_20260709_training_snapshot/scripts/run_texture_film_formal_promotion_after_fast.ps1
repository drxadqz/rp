...
$Log = Join-Path $LogDir "formal_texture_film_candidate_after_fast.log"
$Needed = @(
...
function Wait-ClassificationJobsGone() {
    while ($true) {
        ...
        Write-QueueLog "waiting for classification jobs before texture-FiLM formal launch: $($alive.ProcessId -join ', ')"
        Start-Sleep -Seconds 300
    }
}

Write-QueueLog "started texture-FiLM/Wavelet formal promotion watcher"
Wait-ProcessIds -Ids $WaitPids -Name "texture-FiLM and Wavelet fast queues"
...
while ($true) {
    ...
    Write-QueueLog "waiting for texture-FiLM/Wavelet fast candidates: $($missing -join ', ')"
    Start-Sleep -Seconds 300
}

Write-QueueLog "all texture-FiLM/Wavelet fast candidates present; selecting promotion"
...
if ($null -eq $decision.promoted) {
    Write-QueueLog "no texture-FiLM/Wavelet candidate promoted"
    exit 0
}
...
Write-QueueLog "launching formal texture-FiLM/Wavelet candidate $($decision.promoted.name)"
$stdout = Join-Path $LogDir "formal_texture_film_wavelet_candidate.stdout.log"
$stderr = Join-Path $LogDir "formal_texture_film_wavelet_candidate.stderr.log"
$args = @(
...
Write-QueueLog "formal texture-FiLM/Wavelet python exit code $($proc.ExitCode)"
...
Write-QueueLog "formal texture-FiLM/Wavelet candidate complete"
