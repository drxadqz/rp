...
$Log = Join-Path $LogDir "fast_resolution_queue.log"
...
function Invoke-Fast224Candidate([string]$Name, [string[]]$ExtraArgs) {
...
Write-QueueLog "started 224px resolution fast queue"

Invoke-Fast224Candidate -Name "fast_convnext_tiny_224" -ExtraArgs @()

Invoke-Fast224Candidate -Name "fast_physics_texture_quality_224_region" -ExtraArgs @(
    "--use-physics-branch",
...
Write-QueueLog "224px resolution fast queue complete"
