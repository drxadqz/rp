...
$OutDir = Join-Path $Root "screen_artifact_reliability_film_s4k_cap120_from_current_best_20260703"
...
& $Python scripts\run_rscd_surface_classification.py `
    ...
    --artifact-aware-gate-scale 0.12 `
    --use-texture-film `
    --texture-film-scale 0.06 `
    --train-only-module-prefix artifact_aware_texture_gate `
    --train-only-module-prefix texture_film `
    --train-only-module-prefix norm `
    --online-teacher-checkpoint $Anchor `
    --online-teacher-weight 0.16 `
