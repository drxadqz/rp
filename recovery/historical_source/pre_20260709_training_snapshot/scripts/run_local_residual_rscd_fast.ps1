...
$Log = Join-Path $LogDir "fast_local_residual_queue.log"
...
function Invoke-Candidate([string]$Name, [string[]]$ExtraArgs) {
    ...
    $baseArgs = @(
        ...
        "--output-dir", $outDir,
        "--backbone", "convnext_tiny_local_residual",
        "--embedding-dim", "768",
        "--pretrained",
        "--epochs", "4",
        "--image-size", "224",
...
Write-QueueLog "started local-residual RSCD fast queue"

Invoke-Candidate -Name "fast_convnext_tiny_local_residual_224" -ExtraArgs @()

Invoke-Candidate -Name "fast_physics_texture_local_residual_224_region" -ExtraArgs @(
    "--use-physics-branch",
...
Write-QueueLog "local-residual RSCD fast candidates complete"
