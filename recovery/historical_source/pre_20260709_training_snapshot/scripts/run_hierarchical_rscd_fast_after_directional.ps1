...
$Log = Join-Path $LogDir "fast_hierarchical_smoothing_after_directional.log"
...
function Wait-CommandGone([string]$Pattern, [string]$Name) {
    while ($true) {
        $alive = @(Get-CimInstance Win32_Process | Where-Object { $_.CommandLine -match $Pattern })
        if ($alive.Count -eq 0) {
            break
        }
        Write-QueueLog "waiting for $Name command to finish: $($alive.ProcessId -join ', ')"
        Start-Sleep -Seconds 120
    }
}

function Run-FastCandidate(
    [string]$Name,
    [string[]]$ExtraArgs
) {
    $OutDir = "D:\NMI_SPWFM_datasets\friction_affordance_outputs\rscd_surface_classification\$Name"
...
Write-QueueLog "started hierarchical RSCD fast queue"
Wait-ProcessIds -Ids $FormalWaitPids -Name "formal RSCD jobs"
Wait-CommandGone -Pattern "fast_physics_directional_texture_quality" -Name "directional fast screen"

Run-FastCandidate `
    -Name "fast_physics_texture_hier_smoothing" `
    -ExtraArgs @("--hierarchical-smoothing", "0.08")

Run-FastCandidate `
    -Name "fast_physics_directional_hier_smoothing" `
    -ExtraArgs @("--use-directional-texture-branch", "--directional-texture-dim", "64", "--hierarchical-smoothing", "0.08")

Run-FastCandidate `
    -Name "fast_physics_directional_gated_hier_smoothing" `
    -ExtraArgs @("--use-directional-texture-branch", "--directional-texture-dim", "64", "--use-texture-gate", "--hierarchical-smoothing", "0.08")

Write-QueueLog "all hierarchical candidates complete"
