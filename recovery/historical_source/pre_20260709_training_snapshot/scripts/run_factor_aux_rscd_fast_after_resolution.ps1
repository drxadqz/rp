...
$Log = Join-Path $LogDir "fast_factor_aux_after_resolution.log"
...
Write-QueueLog "started factor-aux RSCD fast watcher"

Wait-File -Path (Join-Path (Join-Path $Root "fast_convnext_tiny_224") "evaluate_test.json") -Name "224px ConvNeXt fast result"
Wait-File -Path (Join-Path (Join-Path $Root "fast_physics_texture_quality_224_region") "evaluate_test.json") -Name "224px PhysicsTexture fast result"

Invoke-Candidate -Name "fast_convnext_tiny_224_factor_aux" -ExtraArgs @(
    "--factor-aux-weight", "0.15"
)

Invoke-Candidate -Name "fast_physics_texture_quality_224_patch_factor_aux" -ExtraArgs @(
    "--use-physics-branch",
...
Write-QueueLog "factor-aux RSCD fast candidates complete"
