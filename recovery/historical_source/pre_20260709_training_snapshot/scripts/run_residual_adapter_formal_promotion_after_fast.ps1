...
$Log = Join-Path $LogDir "formal_residual_adapter_candidate_after_fast.log"
$Needed = @(
    "fast_physics_texture_residual_adapter",
    "fast_physics_directional_residual_adapter"
)
...
function Wait-ClassificationJobsGone() {
    while ($true) {
        ...
        Write-QueueLog "waiting for classification jobs before residual formal launch: $($alive.ProcessId -join ', ')"
        Start-Sleep -Seconds 300
    }
}

Write-QueueLog "started residual-adapter formal promotion watcher"
Wait-ProcessGone -PidValue $ResidualQueuePid -Name "residual-adapter fast queue"
...
while ($true) {
    ...
    Write-QueueLog "waiting for residual-adapter fast candidates: $($missing -join ', ')"
    Start-Sleep -Seconds 300
}

Write-QueueLog "all residual-adapter fast candidates present; selecting promotion"
$decisionText = & $Python scripts\select_rscd_residual_adapter_promotion.py
Write-QueueLog "decision: $decisionText"
$decision = $decisionText | ConvertFrom-Json
if ($null -eq $decision.promoted) {
    Write-QueueLog "no residual-adapter candidate promoted"
    exit 0
}
...
Write-QueueLog "launching formal residual-adapter candidate $($decision.promoted.name)"
$stdout = Join-Path $LogDir "formal_residual_adapter_candidate.stdout.log"
$stderr = Join-Path $LogDir "formal_residual_adapter_candidate.stderr.log"
$args = @(
...
Write-QueueLog "formal residual-adapter python exit code $($proc.ExitCode)"
...
Write-QueueLog "formal residual-adapter candidate complete"
