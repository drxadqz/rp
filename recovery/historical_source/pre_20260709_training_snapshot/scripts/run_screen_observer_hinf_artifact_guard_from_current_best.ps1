...
$OutDir = Join-Path $Root "screen_observer_hinf_artifact_guard_s4k_cap120_from_current_best_20260703"
...
& $Python scripts\run_rscd_surface_classification.py `
    ...
    --train-only-module-prefix physics_branch `
    --train-only-module-prefix semantic_physics_attention_branch `
    --train-only-module-prefix local_physics_field_branch `
    --train-only-module-prefix local_physics_field_adapter `
    --train-only-module-prefix norm `
    --train-only-module-prefix classifier `
    --online-teacher-checkpoint $Anchor `
    --online-teacher-weight 0.06 `
    --online-teacher-temperature 2.0 `
    --online-teacher-beta 4.0 `
    --online-teacher-min-confidence 0.70 `
    --observer-hinf-weight 0.0007 `
    --observer-hinf-mode mixed `
    --observer-hinf-scope wet_water_guarded `
    --observer-hinf-strength 0.85 `
    --observer-hinf-rho 0.18 `
    --observer-hinf-max-lines 4 `
    --observer-hinf-block-ratio 0.14 `
    --observer-hinf-temperature 2.0 `
    --observer-hinf-confidence-threshold 0.55 `
    --observer-hinf-feature-weight 0.10 `
    --observer-hinf-physics-feature-weight 0.55 `
    --observer-hinf-local-physics-feature-weight 0.55 `
    --observer-hinf-low-level-feature-weight 0.20 `
    --observer-hinf-class-consistency-weight 0.04 `
    --observer-hinf-factor-consistency-weight 0.45 `
    --observer-hinf-disturbed-ce-weight 0.06 `
    --observer-hinf-barrier-weight 0.0004 `
    --observer-hinf-barrier-scope wet_water_paved `
    --observer-hinf-barrier-margin-drop 0.04 `
    --relation-conditional-weight 0.0025 `
